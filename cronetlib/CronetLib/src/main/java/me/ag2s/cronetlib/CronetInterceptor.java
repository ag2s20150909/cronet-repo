    ...
    private Response proceedWithCronet(Request request, Call call) throws IOException{
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                NewCallBack newCallBack = new NewCallBack(request, call);
                UrlRequest urlRequest = CronetClient.getInstance().buildRequest(request, newCallBack);
                urlRequest.start();
                return newCallBack.responseFuture.get(40, TimeUnit.SECONDS);
            } else {
