    ...
    private Response proceedWithCronet(Request request, Call call) throws IOException {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                callback = new CronetNewCallBack(request, call, CronetClient.getInstance().getOkhttpClient().eventListenerFactory().create(call));
            } else {
                callback = new CronetCallBack(request, call, CronetClient.getInstance().getOkhttpClient().eventListenerFactory().create(call));
            }
